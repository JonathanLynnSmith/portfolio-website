<script>
    ...
    const handleScroll = () => {
        if (window.scrollY > 50) {
            // You can adjust the scroll value as needed
            isScrolled = true;
        } else {
            isScrolled = false;
        }
...
<header
    class={`sticky top-0 z-50 transition-all duration-300 ${isScrolled ? "bg-white shadow-md" : "bg-transparent"} ${isScrolled ? "h-10 py-1 px-4" : "h-24 py-6"}`}
>
